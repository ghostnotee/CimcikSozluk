...
namespace CimcikSozluk.Api.Application.Features.Commands.User.ConfirmEmail;
...
public class ConfirmEmailCommandHandler : IRequestHandler<ConfirmEmailCommand, bool>
{
    ...
    public async Task<bool> Handle(ConfirmEmailCommand request, CancellationToken cancellationToken)
    {
        ...
        if (confirmation is null) throw new DatabaseValidationException("Confirmation not found");
        var dbUser = await _userRepository.GetSingleAsync(i => i.EmailAddress == confirmation.NewEmailAddress);
        if (dbUser is null) throw new DatabaseValidationException("user not found with this email");
        if (dbUser.EmailConfirmed) throw new DatabaseValidationException("Email address is already confirmed!");
